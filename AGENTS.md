...
- For broader repo-understanding tasks spanning many files or asking for cross-file relationships, prefer the repo-local `repo-graph` workflow when available before falling back to repeated file reads or broad whole-repo reads.
- Use `repo-graph` by default when the request is about how concepts connect, what bridges two areas, what depends on what across files, or when no single concrete file/symbol anchor is obvious.
- Keep `omx explore` or direct reads as the default when the task already names a concrete file, symbol, error site, or a tiny bounded lookup target.
- For repo-understanding, do not default to grep/raw shell as the conceptual search strategy; reserve grep/raw shell for mechanical verification or exact literal/procedural checks.

When to use what:
- Use `omx explore --prompt ...` for simple read-only lookups.
- Use the repo-local `repo-graph` capability for non-trivial repo-understanding work that benefits from graph-backed bounded context.
- Use `omx sparkshell` for noisy read-only shell commands, bounded verification runs, repo-wide listing/search, or tmux-pane summaries; `omx sparkshell --tmux-pane ...` is explicit opt-in. Do not treat sparkshell/grep as the default repo-understanding path when repo-graph fits.
...
- If `repo-graph` returns weak signal, fall back to `omx explore` or direct file reads instead of forcing graph usage.
